...
const settings = {
  squareSize: 80,
  // one square, two faces: touch feels floaty, pen feels deliberate
  squarePresets: [
...
function makeSquare(presets) {
  return { presets, virtX: 0, virtY: 0, realX: 0, realY: 0,
           velX: 0, velY: 0, isDragging: false, pressure: 0, lastEvent: /** @type {PointerEvent|null} */ (null) };
}
...
function updateDebug(e) {
  ...
  const preset = sq ? `${e.pointerType}→${activePreset}` : `–`;
  debug.textContent =
