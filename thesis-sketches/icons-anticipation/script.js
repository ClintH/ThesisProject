...
const buttons = Array.from({ length: settings.count }, () => {
  const el = document.createElement('button');
  el.style.cssText = `
    width:100px; height:100px; border-radius:12px;
    border:0.5px solid rgba(0,0,0,0.18); background:rgba(0,0,0,0.18);
...
continuously(() => {
  ...
  for (const { el, s } of buttons) {
    ...
    el.style.transform = `translateY(${Math.round(s.liftY * 10) / 10}px) scaleY(${Math.round(s.scaleY * 1000) / 1000})`;
