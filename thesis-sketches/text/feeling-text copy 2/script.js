const settings = {
  ...
  edgeRadius:     28,     // px — how far from a word edge resistance begins
  edgeDamping:    0.38,   // damping right at an edge  — strong resistance
  freeDamping:    0.88,   // damping far from edges    — free movement
...
function updateDebug() {
  debug.textContent =
    ...
    `edge ${Math.round(nearestBoundary(state.cursorX).dist)}px  ·  ` +
...
function draw() {
  ...
  // ── Physics: spring pulls cursor toward pointer, drag peaks near word edges
if (dragging) {
  const { boundary, dist } = nearestBoundary(state.cursorX);
  const t      = clamp(dist / settings.edgeRadius, 0, 1);
  const smooth = t * t * (3 - 2 * t);          // 0 = on boundary, 1 = far away

  const snapForce = (1 - smooth) * (boundary - state.cursorX) * 0.4;

  state.velocity += (state.targetX - state.cursorX) * settings.springStrength;
  state.velocity += snapForce;                  // pulls cursor onto boundary
  const damping   = smooth * settings.freeDamping + (1 - smooth) * settings.edgeDamping;
